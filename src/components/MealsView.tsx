...
import { Plus, X, UtensilsCrossed, Droplets, Trash2, Timer, Play, Square, Pencil, ChevronLeft, ChevronRight } from 'lucide-react';
...
function FastingView() {
    ...
    const load = async () => {
        ...
        try {
            ...
            setActive(a);
            setHistory(h.filter((s: FastingSession) => s.status !== 'active'));
    ...
    // Group history by "YYYY-MM" for display
    const grouped = history.reduce<Record<string, FastingSession[]>>((acc, s) => {
        const key = format(new Date(s.startTime), 'yyyy-MM');
        if (!acc[key]) acc[key] = [];
        acc[key].push(s);
        return acc;
    }, {});
    ...
    const totalDays = new Set(completedSessions.map(s => format(new Date(s.startTime), 'yyyy-MM-dd'))).size;
            ...
            {history.length > 0 && (
                <div>
                    ...
                    <div className="card" style={{ marginBottom: 14 }}>
                        <div style={{ fontWeight: 700, marginBottom: 12 }}>All-time Stats</div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
                            <div className="stat-box">
                                <span className="stat-label">Fasting Days</span>
                                <span className="stat-value" style={{ color: '#8b5cf6' }}>{totalDays}</span>
                                <span className="stat-unit">days</span>
                            </div>
                            <div className="stat-box">
                                <span className="stat-label">Total Hours</span>
                                <span className="stat-value" style={{ color: '#8b5cf6' }}>{totalHours.toFixed(1)}</span>
                    ...
                    {groupKeys.map(key => {
                        ...
                        const monthCompleted = sessions.filter(s => s.status === 'completed');
                        const monthDays = new Set(monthCompleted.map(s => format(new Date(s.startTime), 'yyyy-MM-dd'))).size;
                        const monthHours = monthCompleted.reduce((sum, s) => {
                            if (!s.endTime) return sum;
                            return sum + (new Date(s.endTime).getTime() - new Date(s.startTime).getTime()) / 3600000;
                        }, 0);
                        return (
                            <div key={key} style={{ marginBottom: 16 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                                    <span style={{ fontWeight: 700, fontSize: '0.875rem', color: 'var(--color-text-2)' }}>
                                        {format(new Date(key + '-01'), 'MMMM yyyy')}
                                    </span>
                                    <span style={{ fontSize: '0.75rem', color: 'var(--color-text-3)' }}>
                                        {monthDays} day{monthDays !== 1 ? 's' : ''} · {monthHours.toFixed(1)}h
                                    </span>
                                </div>
                                {sessions.map(s => {
            ...
            {tab === 'log' && (
                <div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                        <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setDate(format(subDays(new Date(date), 1), 'yyyy-MM-dd'))}>
                            <ChevronLeft size={16} />
                        </button>
                        <div style={{ flex: 1 }}>
                            <DatePicker value={date} onChange={v => v && setDate(v)} />
                        </div>
                        <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setDate(format(addDays(new Date(date), 1), 'yyyy-MM-dd'))}>
